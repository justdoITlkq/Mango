...
public class GifImageDecoder {
  ...
  @SuppressLint("NewApi") private Bitmap extractImage() {
    ...
    try {
      ...
      Bitmap newBitmap = BitmapFactory.decodeStream(
          new BufferedInputStream(new ByteArrayInputStream(outputStream.toByteArray())), null,
          options);//加上图片缩略
      if (newBitmap != null) {
        if (mLastImage == null) {
          return newBitmap;
  ...
  private class GifHeader {
    ...
    public int[] getGlobalColorTable() {
      ...
      for (int i = 0; i < colors.length; i++) {
        colors[i] =
            ((bytes[13 + (i * 3)] & 0xFF) << 16) + ((bytes[13 + (i * 3) + 1] & 0xFF) << 8) + (bytes[
                13
                    + (i * 3)
                    + 2] & 0xFF);
      }
  ...
  private class ImageBlock {
    ...
    public int[] getLocalColorTable() {
      ...
      for (int i = 0; i < colors.length; i++) {
        colors[i] =
            ((bytes[10 + (i * 3)] & 0xFF) << 16) + ((bytes[10 + (i * 3) + 1] & 0xFF) << 8) + (bytes[
                10
                    + (i * 3)
                    + 2] & 0xFF);
      }
