package com.handsomeyang.mango.image.frescoUtils;
...
public class FrescoUtils {
  ...
  private Bitmap fastBlur(Bitmap bkg, int radius, int downSampling) {
    ...
    Bitmap smallBitmap = Bitmap.createScaledBitmap(bkg, bkg.getWidth() / downSampling,
        bkg.getHeight() / downSampling, true);
  ...
  /**
   * If the image has some ResizeOptions we put also the resized image into the cache with
   * different
   * key.
   * currently don't support downsampling / resizing for GIFs.
   */
  public static void loadUrl(String url, SimpleDraweeView draweeView, BasePostprocessor processor,
      int width, int height, BaseControllerListener listener) {

    load(Uri.parse(url), draweeView, processor, width, height, listener);
  }

  public static void loadFile(String file, SimpleDraweeView draweeView, BasePostprocessor processor,
      int width, int height, BaseControllerListener listener) {

    load(getFileUri(file), draweeView, processor, width, height, listener);
  }

  public static void loadFile(File file, SimpleDraweeView draweeView, BasePostprocessor processor,
      int width, int height, BaseControllerListener listener) {

    load(getFileUri(file), draweeView, processor, width, height, listener);
  }

  public static void loadRes(int resId, SimpleDraweeView draweeView, BasePostprocessor processor,
      int width, int height, BaseControllerListener listener) {

    load(getResUri(resId), draweeView, processor, width, height, listener);
  }

  public static void load(Uri uri, SimpleDraweeView draweeView, BasePostprocessor processor,
      int width, int height, BaseControllerListener listener) {
   ...
   * 只有BitmapDrawable 和 ColorDrawable类的图片可以实现圆角。我们目前不支持包括NinePatchDrawable和
   * ShapeDrawable在内的其他类型图片。（无论他们是在XML或是程序中声明的）
   * 动画不能被圆角。
  ...
  public static File getFileFromDiskCache(String url) {
    ...
    if (!TextUtils.isEmpty(url)) {
      CacheKey cacheKey =
          //杨晓彬在这添加了参数null
          DefaultCacheKeyFactory.getInstance().getEncodedCacheKey(ImageRequest.fromUri(url), null);
  ...
  public static boolean copyCacheFile(String url, File path) {
    ...
    if (path.isDirectory()) {
      throw new RuntimeException(
          path + "is a directory,you should call copyCacheFileToDir(String url,File dir)");
    }
  ...
  public static File copyCacheFileToDir(String url, File dir) {
    ...
    if (!dir.isDirectory()) {
      throw new RuntimeException(
          dir + "is not a directory,you should call copyCacheFile(String url,File path)");
    }
    if (!dir.exists()) {
      dir.mkdirs();
    }
   ...
   */
  public static void download(final String url, Context context, final File dir,
   ...
   * 拿到指定宽高的bitmap
   */
  public static void getBitmap(String url, Context context, int width, int height,
   ...
   */
  public static void getBitmapWithProcessor(final String url, Context context, final int width,
      final int height, BasePostprocessor processor, final BitmapListener listener) {
    ...
    dataSource.subscribe(new BaseBitmapDataSubscriber() {
      @Override protected void onNewResultImpl(Bitmap bitmap) {
        ...
        if (url.contains("gif")) {
          ...
          if (cacheFile.exists()) {
            Bitmap bitmapGif =
                //杨晓彬在这添加了参数 with 和  height
                GifUtils.getBitmapFromGifFile(cacheFile, width, height);//拿到gif第一帧的bitmap
            Bitmap target =
                MyBitmapUtils.compressBitmap(bitmapGif, true, width, height);//将bitmap压缩到指定宽高。
            if (target != null) {
  ...
  public static void loadUrlWithFace(final String url, final SimpleDraweeView draweeView) {
    ...
    draweeView.getHierarchy().setActualImageFocusPoint(pointF);
  ...
  public static PointF getFaceFocusRatio(Bitmap bitmap) {
    ...
    //Logger.e(
    //    bitmap.getWidth() + "--width---heitht:" + bitmap.getHeight() + "---count:" + faceCount);
    L.e(bitmap.getWidth() + "--width---heitht:" + bitmap.getHeight() + "---count:" + faceCount);
    if (faceCount == 1) {
  ...
  public static class MyImageCacheStatsTracker implements ImageCacheStatsTracker {
    @Override public void onBitmapCachePut() {

    }

    @Override public void onBitmapCacheHit() {

    }

    @Override public void onBitmapCacheMiss() {

    }

    @Override public void onMemoryCachePut() {

    }

    @Override public void onMemoryCacheHit() {

    }

    @Override public void onMemoryCacheMiss() {

    }

    @Override public void onStagingAreaHit() {

    }

    @Override public void onStagingAreaMiss() {

    }

    @Override public void onDiskCacheHit() {
      //Logger.e("ImageCacheStatsTracker---onDiskCacheHit");
    }

    @Override public void onDiskCacheMiss() {
      //Logger.e("ImageCacheStatsTracker---onDiskCacheMiss");
    }

    @Override public void onDiskCacheGetFail() {
