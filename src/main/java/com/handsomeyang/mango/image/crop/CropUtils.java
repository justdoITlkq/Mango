package com.handsomeyang.mango.image.crop;
...
public class CropUtils {
  ...
  private static Uri buildUri() {
    ...
    if (!cacheFolder.exists()) {
      try {
        ...
        Log.d("uri",
            "generateUri " + cacheFolder + " result: " + (result ? "succeeded" : "failed"));
      } catch (Exception e) {
   ...
   * 注意，调用时data为null的判断
   */
  public static void handleResult(Activity context, CropHandler cropHandler, int requestCode,
    ...
    if (resultCode == Activity.RESULT_OK) {
      if (requestCode == REQUEST_SELECT_PICTURE) {//第一次，选择图片后返回
        ...
        cropHandler.handleCropResult(finalUri, config.tag);
          ...
          == REQUEST_CAMERA) {//第一次，拍照后返回，因为设置了MediaStore.EXTRA_OUTPUT，所以data为null，数据直接就在uri中
