...
package com.handsomeyang.mango.thrid.observablescrollview;
...
public class ObservableGridView extends GridView implements Scrollable {
  ...
  @Override public void addScrollViewCallbacks(ObservableScrollViewCallbacks listener) {
  ...
  @Override public void removeScrollViewCallbacks(ObservableScrollViewCallbacks listener) {
    if (mCallbackCollection != null) {
      mCallbackCollection.remove(listener);
    }
  }

  @Override public void clearScrollViewCallbacks() {
    if (mCallbackCollection != null) {
  ...
  public boolean removeHeaderView(View v) {
    if (mHeaderViewInfos.size() > 0) {
      ...
      ListAdapter adapter = getAdapter();
      if (adapter != null
          && adapter instanceof HeaderViewGridAdapter
  ...
  private void onScrollChanged() {
    ...
    if (getChildCount() > 0) {
      ...
      for (int i = getFirstVisiblePosition(), j = 0; i <= getLastVisiblePosition(); i++, j++) {
        if (mChildrenHeights.indexOfKey(i) < 0 || getChildAt(j).getHeight() != mChildrenHeights.get(
            i)) {
  ...
  public static class HeaderViewGridAdapter implements WrapperListAdapter, Filterable {
    ...
    public HeaderViewGridAdapter(ArrayList<FixedViewInfo> headerViewInfos,
      ...
      mAreAllFixedViewsSelectable =
          areAllListInfosSelectable(mHeaderViewInfos) && areAllListInfosSelectable(
              mFooterViewInfos);
    }
    ...
    @Override public boolean isEnabled(int position) {
      ...
      return footerPosition % mNumColumns == 0 && mFooterViewInfos.get(
          footerPosition / mNumColumns).isSelectable;
    }
    ...
    @Override public int getItemViewType(int position) {
      ...
      if (mCachePlaceHoldView) {
        ...
        final int footerPosition = adjPosition - adapterCount;
        if (footerPosition >= 0
            && footerPosition < getCount()
            && (footerPosition % mNumColumns) != 0) {
          type = adapterViewTypeStart + mHeaderViewInfos.size() + 1 + (footerPosition / mNumColumns
              + 1);
        }
      }
