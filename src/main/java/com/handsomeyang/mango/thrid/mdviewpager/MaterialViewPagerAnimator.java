...
import com.handsomeyang.mango.thrid.observablescrollview.ObservableScrollView;
import com.handsomeyang.mango.thrid.observablescrollview.ObservableScrollViewCallbacks;
import com.handsomeyang.mango.thrid.observablescrollview.ObservableWebView;
import com.handsomeyang.mango.thrid.observablescrollview.ScrollState;
...
import static com.handsomeyang.mango.thrid.mdviewpager.Utils.*;
...
public class MaterialViewPagerAnimator {
...

    {
      ...
      if (mHeader.headerBackground != null) {
        ...
        if (this.settings.parallaxHeaderFactor != 0) {
          ViewCompat.setTranslationY(mHeader.headerBackground,
              scrollTop / this.settings.parallaxHeaderFactor);
        }
    ...
    percent = minMax(0, percent, 1);
    {
      ...
      if (mHeader.mLogo != null) { //move the header logo to toolbar
        ...
        if (this.settings.hideLogoWithFade) {
          ...
          ViewCompat.setTranslationY(mHeader.mLogo,
              (mHeader.finalTitleY - mHeader.originalTitleY) * percent);
        } else {
          ViewCompat.setTranslationY(mHeader.mLogo,
              (mHeader.finalTitleY - mHeader.originalTitleY) * percent);
          ViewCompat.setTranslationX(mHeader.mLogo,
              (mHeader.finalTitleX - mHeader.originalTitleX) * percent);
  ...
  public void setColorPercent(float percent) {
    ...
    setBackgroundColor(colorWithAlpha(this.settings.color, percent), mHeader.statusBackground);
   ...
   * Listen to RecyclerView.OnScrollListener so give to $[onScrollListener] your
   * RecyclerView.OnScrollListener if you already use one
   ...
   * Listen to ObservableScrollViewCallbacks so give to $[observableScrollViewCallbacks] your
   * ObservableScrollViewCallbacks if you already use one
   * For loadmore or anything else
   *
  ...
  public void registerScrollView(final ObservableScrollView scrollView,
    ...
    if (scrollView != null) {
      ...
      if (scrollView.getParent() != null && scrollView.getParent().getParent() != null && scrollView
          .getParent()
      ...
      scrollView.setScrollViewCallbacks(new ObservableScrollViewCallbacks() {
        ...
        @Override public void onScrollChanged(int yOffset, boolean b, boolean b2) {
  ...
  private void animateEnterToolbarLayout(float yOffset) {
    ...
    if (headerAnimator == null) {
      if (Build.VERSION.SDK_INT > Build.VERSION_CODES.GINGERBREAD_MR1) {
        headerAnimator =
            android.animation.ObjectAnimator.ofFloat(mHeader.toolbarLayout, "translationY", 0)
                .setDuration(ENTER_TOOLBAR_ANIMATION_DURATION);
        ((android.animation.ObjectAnimator) headerAnimator).addListener(
        ...
        headerAnimator = ObjectAnimator.ofFloat(mHeader.toolbarLayout, "translationY", 0)
            .setDuration(ENTER_TOOLBAR_ANIMATION_DURATION);
