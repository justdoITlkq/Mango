...
public class MaterialViewPager extends FrameLayout implements ViewPager.OnPageChangeListener {
  ...
  public void setImageUrl(String imageUrl, int fadeDuration) {
    if (imageUrl != null) {
      final ImageView headerBackgroundImage =
          (ImageView) findViewById(R.id.materialviewpager_imageHeader);
  ...
  public void setImageDrawable(Drawable drawable, int fadeDuration) {
    if (drawable != null) {
      ...
      if (headerBackgroundImage != null) {
        ViewCompat.setAlpha(headerBackgroundImage, settings.headerAlpha);
        MaterialViewPagerImageHelper.setImageDrawable(headerBackgroundImage, drawable,
            fadeDuration);
        setImageHeaderDarkLayerAlpha();
      }
    }
  ...
  @Override protected void onFinishInflate() {
    ...
    { //replace the viewpager ?
      ...
      if (viewPagerLayoutId != -1) {
        ...
        viewpagerContainer.addView(LayoutInflater.from(getContext())
...

    {
      ...
      headerBackgroundContainer.addView(
          LayoutInflater.from(getContext()).inflate(headerId, headerBackgroundContainer, false));
    }
  ...
  private void initialiseHeights() {
    if (headerBackground != null) {
      ...
      layoutParams.height =
          (int) Utils.dpToPx(this.settings.headerHeight + settings.headerAdditionalHeight,
              getContext());
