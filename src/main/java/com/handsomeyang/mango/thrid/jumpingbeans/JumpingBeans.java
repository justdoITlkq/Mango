...
package com.handsomeyang.mango.thrid.jumpingbeans;
...
public final class JumpingBeans {
  ...
  public static class Builder {
     ...
     */
    @NonNull public Builder appendJumpingDots() {
    ...
    private static CharSequence appendThreeDotsEllipsisTo(TextView textView) {
      ...
      if (!endsWithThreeEllipsisDots(text)) {
        text = new SpannableStringBuilder(text).append(
            THREE_DOTS_ELLIPSIS);  // Preserve spans in original text
      }
      return text;
    }
    ...
    private static boolean endsWithThreeEllipsisDots(CharSequence text) {
      ...
      CharSequence lastThreeChars =
          text.subSequence(text.length() - THREE_DOTS_ELLIPSIS_LENGTH, text.length());
     ...
     * @see #setIsWave(boolean)
     */
    @NonNull public Builder makeTextJump(@IntRange(from = 0) int startPos,
        @IntRange(from = 0) int endPos) {
      CharSequence text = textView.getText();
     ...
     */
    @NonNull public Builder setAnimatedDutyCycle(
        @FloatRange(from = 0f, to = 1f, fromInclusive = false) float animatedRange) {
     ...
     */
    @NonNull public Builder setLoopDuration(@IntRange(from = 1) int loopDuration) {
      if (loopDuration < 1) {
     ...
     * @see #setIsWave(boolean)
     */
    @NonNull public Builder setWavePerCharDelay(@IntRange(from = 0) int waveCharOffset) {
      if (waveCharOffset < 0) {
     ...
     */
    @NonNull public Builder setIsWave(boolean wave) {
      this.wave = wave;
      return this;
    }
     ...
     * free up memory and CPU that would be otherwise wasted.
     */
    @NonNull public JumpingBeans build() {
