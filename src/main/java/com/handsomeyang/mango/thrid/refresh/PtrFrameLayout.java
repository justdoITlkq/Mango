package com.handsomeyang.mango.thrid.refresh;
...
import com.handsomeyang.mango.thrid.refresh.indicator.PtrIndicator;
import com.handsomeyang.mango.thrid.refresh.util.PtrCLog;

/**
 * This layout view for "Pull to Refresh(Ptr)" support all of the view, you can contain everything
 * you want.
 * support: pull to refresh / release to refresh / auto refresh / keep header view while refreshing
 * / hide header view while refreshing
 * It defines {@link .PtrUIHandler}, which allows you customize the UI
 * easily.
 */
public class PtrFrameLayout extends ViewGroup {
    ...
    if (mContent != null) {
      ...
      if (isDebug()) {
        ...
        PtrCLog.d(LOG_TAG, "onMeasure content, width: %s, height: %s, margin: %s %s %s %s",
            getMeasuredWidth(), getMeasuredHeight(), lp.leftMargin, lp.topMargin, lp.rightMargin,
            lp.bottomMargin);
  ...
  private void updatePos(int change) {
    ...
    if (DEBUG) {
      PtrCLog.v(LOG_TAG, "updatePos: change: %s, current: %s last: %s, top: %s, headerHeight: %s",
          change, mPtrIndicator.getCurrentPosY(), mPtrIndicator.getLastPosY(), mContent.getTop(),
          mHeaderHeight);
    }
   ...
   * The UI will perform complete at once or after a delay, depends on the time elapsed is greater
   * then {@link #mLoadingMinTime} or not.
   */
