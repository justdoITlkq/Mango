package com.handsomeyang.mango.thrid.bottombar;
...
public class BottomBar extends LinearLayout
  ...
  private void populateAttributes(Context context, AttributeSet attrs) {
    ...
    try {
      ...
      @ColorInt int defaultInActiveColor = isShiftingMode() ? Color.WHITE
  ...
  private void determineInitialBackgroundColor() {
    ...
    boolean userHasDefinedBackgroundColor =
        userDefinedBackground != null && userDefinedBackground instanceof ColorDrawable;
  ...
  private BottomBarTab.Config getTabConfig() {
    return new BottomBarTab.Config.Builder().inActiveTabAlpha(inActiveTabAlpha)
  ...
  private void resizeTabsToCorrectSizes(List<BottomBarTab> bottomBarItems,
      BottomBarTab[] viewsToAdd) {
    int proposedItemWidth =
        Math.min(MiscUtils.dpToPixel(getContext(), screenWidth / bottomBarItems.size()),
            maxFixedItemWidth);

    inActiveShiftingItemWidth = (int) (proposedItemWidth * 0.9);
    activeShiftingItemWidth =
        (int) (proposedItemWidth + (proposedItemWidth * (bottomBarItems.size() * 0.1)));
   ...
   */
  @IdRes public int getCurrentTabId() {
  ...
  private void updateShyHeight(int height) {
    ((CoordinatorLayout.LayoutParams) getLayoutParams()).setBehavior(
        new BottomNavigationBehavior(height, 0, false));
  }
  ...
  @VisibleForTesting Bundle saveState() {
  ...
  @VisibleForTesting void restoreState(Bundle savedInstanceState) {
    if (savedInstanceState != null) {
      isComingFromRestoredState = true;
      ignoreTabReselectionListener = true;

      int restoredPosition =
          savedInstanceState.getInt(STATE_CURRENT_SELECTED_TAB, currentTabPosition);
