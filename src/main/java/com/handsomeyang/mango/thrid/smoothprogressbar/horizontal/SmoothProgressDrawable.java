package com.handsomeyang.mango.thrid.smoothprogressbar.horizontal;
...
import static com.handsomeyang.mango.thrid.smoothprogressbar.horizontal.SmoothProgressBarUtils.checkColors;
import static com.handsomeyang.mango.thrid.smoothprogressbar.horizontal.SmoothProgressBarUtils.checkNotNull;
import static com.handsomeyang.mango.thrid.smoothprogressbar.horizontal.SmoothProgressBarUtils.checkPositive;
import static com.handsomeyang.mango.thrid.smoothprogressbar.horizontal.SmoothProgressBarUtils.checkPositiveOrZero;
import static com.handsomeyang.mango.thrid.smoothprogressbar.horizontal.SmoothProgressBarUtils.checkSpeed;
...
public class SmoothProgressDrawable extends Drawable implements Animatable {
  ...
  private SmoothProgressDrawable(Interpolator interpolator, int sectionsCount, int separatorLength,
      int[] colors, float strokeWidth, float speed, float progressiveStartSpeed,
      float progressiveStopSpeed, boolean reversed, boolean mirrorMode, Callbacks callbacks,
      boolean progressiveStartActivated, Drawable backgroundDrawable, boolean useGradients) {
    mRunning = false;
  ...
  public void setSeparatorLength(int separatorLength) {
    if (separatorLength < 0) throw new IllegalArgumentException("SeparatorLength must be >= 0");
    mSeparatorLength = separatorLength;
    invalidateSelf();
  }
  ...
  private void drawGradient(Canvas canvas) {
    ...
    float left =
        mReversed ? (mMirrorMode ? Math.abs(mBounds.left - mBounds.right) / 2 : mBounds.left)
            : mBounds.left;
    float right =
        mMirrorMode ? (mReversed ? mBounds.left : Math.abs(mBounds.left - mBounds.right) / 2)
            : mBounds.right;
    float top = mBounds.centerY() - mStrokeWidth / 2;
    float bottom = mBounds.centerY() + mStrokeWidth / 2;
    LinearGradient linearGradient =
        new LinearGradient(left, top, right, bottom, mLinearGradientColors,
            mLinearGradientPositions, mMirrorMode ? Shader.TileMode.MIRROR : Shader.TileMode.CLAMP);
  ...
  private void drawBackground(Canvas canvas, float fromX, float toX) {
    ...
    canvas.clipRect(fromX, (int) ((canvas.getHeight() - mStrokeWidth) / 2), toX,
        (int) ((canvas.getHeight() + mStrokeWidth) / 2));
  ...
  public static class Builder {
    ...
    public SmoothProgressDrawable build() {
      ...
      SmoothProgressDrawable ret =
          new SmoothProgressDrawable(mInterpolator, mSectionsCount, mStrokeSeparatorLength, mColors,
              mStrokeWidth, mSpeed, mProgressiveStartSpeed, mProgressiveStopSpeed, mReversed,
              mMirrorMode, mOnProgressiveStopEndedListener, mProgressiveStartActivated,
              mBackgroundDrawableWhenHidden, mGradients);
      return ret;
    }
